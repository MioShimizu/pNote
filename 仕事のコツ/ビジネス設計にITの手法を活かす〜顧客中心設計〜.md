...
次の作業は表（マトリックス/マップ）での整理になる。表の横軸はセールスからサービス提供とフォローアップまでの時間軸（ステップ/フェーズ）、縦軸は登場人物とその行動と感情になる。※以下例を図示するが著作権を無視した無許可転載なので一見たら削除するので連絡してください。  
